        ...
        <div class="mx-auto lg:pl-0 md:pl[80px] pl-0">
            ...
            <Slide v-for="slide in allUsers" :key="slide">
                <Link :href="route('users.show', {id: slide.id})" class="relative mx-auto text-center mt-4 px-2 cursor-pointer">
                    <div class="absolute z-[-1] -top-[5px] left-[4px] rounded-full rotate-45 w-[64px] h-[64px] contrast-[1.3] bg-gradient-to-t from-[#EEBE13] to-[#0038FF] via-[#258334]">
            ...
            <div id="Posts" class="px-4 max-w-[600px] mx-auto mt-10" v-for="post in posts.data" :key="post">
                <div class="flex items-center justify-between py-2 bg-light rounded-t-[20px] px-3">
                ...
                <div class="bg-dark rounded-lg w-full min-h-[400px] flex items-center">
                    <img class="w-full mx-auto" :src="post.file">
                </div>

                <div class="bg-light rounded-b-[20px] px-3 py-2">
                    <LikesSection 
                        :post="post"
                        @like="updateLike($event)"
                    />

                    <div class="text-dark font-bold py-1">{{ post.likes.length }} likes</div>
                    <div>
                        <span class="text-dark font-bold">{{ post.user.name }}</span>
                        {{ post.text }}
                    </div>
                    <button 
