...
<template>
    <div id="OverlaySection" class="fixed z-50 top-0 left-0 w-full h-screen bg-[#000000] bg-opacity-60 p-3">
        ...
        <div class="max-w-6xl h-[calc(100%-100px)] mx-auto mt-10 bg-light rounded-xl">
            <div class="flex items-center justify-between w-full rounded-t-xl p-5 border-b border-b-gray-300">
                <ArrowLeft :size="30" fillColor='#000000' @click="closeOverlay()" class="cursor-pointer"/>
                <div class="font-extrabold text-lg">Create New Post</div>
            ...
            <div class="w-full md:flex h-[calc(100%-55px)] rounded-xl overflow-auto">
                <div class="flex items-center bg-gray-100 w-full h-full overflow-hidden">
                    <div v-if="!fileDisplay" class="flex flex-col items-center mx-auto">
                        <label
                            for="file"
                            class="hover:bg-blue-700 bg-blue-500 rounded-lg p-2.5 text-light font-extrabold cursor-pointer"
                        >
                        ...
                        >
                        <div v-if="error && error.file" class="text-red-500 text-center p-2 font-bold">{{ error.file }}</div>
                        <div v-if="!fileDisplay && isValidFile === false" class="text-red-500 text-center p-2 font-bold">
                            File not accepted
                        </div>
                    </div>
                ...
                <div id="TextAreaSection" class="max-w-[720px] w-full relative">
                    <div class="flex items-center justify-between p-3">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4 font-bold text-[15px]">{{ user.name }}</div>
                        </div>
                    </div>
                    <div v-if="error && error.text" class="text-red-500 p-2 font-bold">{{ error.text }}</div>

                    <div class=" flex w-full max-h-[200px] bg-light border-b">
                        <textarea
                        ref="textarea"
                        v-model="form.text"
                        placeholder="Write a caption..."
                        rows="10"
                        class="placeholder-gray-500 w-full border-0 mt-2 mb-2 z-50 focus:ring-0 text-gray-600 text-[16px]"
                        ></textarea>
                    </div>

                    <div class="flex items-center justify-between border-b p-3">
                        <div class="text-lg font-extrabold text-gray-500">Add Location</div>
                        <MapMarkerOutline :size="27" fillColor='#000000' />
                    </div>
                    <div class="flex items-center justify-between border-b p-3">
                        <div class="text-lg font-extrabold text-gray-500">Accesibility</div>
                        <ChevronDown :size="27" fillColor='#000000' />
                    </div>
                    <div class="flex items-center justify-between border-b p-3">
                    ...
                    <div class="text-gray-500 mt-3 p-3 text-sm">
                        ...
                        It may also appear in places such as Explore, where anyone can see it.
                    </div>
                </div>
            </div>
        </div>
